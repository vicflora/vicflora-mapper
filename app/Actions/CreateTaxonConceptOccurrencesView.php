...
class CreateTaxonConceptOccurrencesView
{
    ...
    {
        $sql = <<<SQL
CREATE OR REPLACE VIEW mapper.taxon_occurrences_view
 AS
 SELECT tco.taxon_concept_id,
    t.scientific_name,
    ...
    COALESCE(aocc.asserted_value, t.occurrence_status, 'present'::character varying) AS occurrence_status,
        CASE
            ...
            WHEN t.occurrence_status IS NOT NULL AND t.occurrence_status::text <> ''::text THEN 'taxon'::text
            ELSE NULL::text
        END AS occurrence_status_source,
    COALESCE(aest.asserted_value, o.establishment_means, t.establishment_means, 'native'::character varying) AS establishment_means,
        CASE
            ...
            CASE o.data_resource_uid
                ...
                ELSE 'AVH'::text
            END
            WHEN t.establishment_means IS NOT NULL AND t.establishment_means::text <> ''::text THEN 'taxon'::text
            ELSE NULL::text
        END AS establishment_means_source,
    COALESCE(adeg.asserted_value, o.degree_of_establishment, t.degree_of_establishment, 'native'::character varying) AS degree_of_establishment,
        CASE
            ...
            CASE o.data_resource_uid
                ...
                ELSE 'AVH'::text
            END
            WHEN t.degree_of_establishment IS NOT NULL AND t.degree_of_establishment::text <> ''::text THEN 'taxon'::text
            ELSE NULL::text
