...
class LoadOccurrences
{
     ...
     */
    public function __invoke(int $pageSize): void
    {
        $count = $pageSize;
        $start = 1;
        $end = $pageSize;
        while ($count == $pageSize) {
            $occurrences = DB::connection('vicfunga')
                ->query()
                ->select(
                    'd.uuid as id',
            ...
            DB::connection('vicfunga')->table('mapper.occurrences')->insert($insertData);
        }
    }
}
